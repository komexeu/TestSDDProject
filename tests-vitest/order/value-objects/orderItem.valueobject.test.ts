...
// Value Object 測試：OrderItem

describe('OrderItem', () => {
  it('should create a valid OrderItem', () => {
    const item = new OrderItem('p1', '珍奶', 2, 50);
  ...
  it('should throw if quantity <= 0', () => {
    expect(() => new OrderItem('p1', '珍奶', 0, 50)).toThrow();
  });

  it('should throw if price < 0', () => {
    expect(() => new OrderItem('p1', '珍奶', 1, -10)).toThrow();
  });

  it('should be equal if all values are the same', () => {
    const a = new OrderItem('p1', '珍奶', 1, 50);
    const b = new OrderItem('p1', '珍奶', 1, 50);
    expect(a.equals(b)).toBe(true);
  });

  it('withQuantity should return new OrderItem with updated quantity', () => {
    const item = new OrderItem('p1', '珍奶', 1, 50);
